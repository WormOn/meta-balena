...
def resinos_build_configuration():
    ...
    if d.getVar('BALENA_STORAGE', True) not in ['aufs', 'overlay2']:
        ...
        success = False
    if d.getVar('BALENA_CONNECTABLE', True) or d.getVar('BALENA_CONNECTABLE_SERVICES', True) or d.getVar('BALENA_CONNECTABLE_ENABLE_SERVICES', True):
        bb.warn("Your build configuration uses BALENA_CONNECTABLE* variables. These variables are no longer used. There is only one type of resinOS image type which is unconnected by default. The os-config tool is used to configure the resinOS image for connectivity to a resin instance.")
